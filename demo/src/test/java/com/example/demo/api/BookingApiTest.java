...
/**
 * اختبار تكاملي على /api/bookings مع اعتماد JWT Security: 1. نطلب /auth/login للحصول على توكن 2.
 * نستخدمه في استدعاء POST /api/bookings 3. نتأكد أننا نحصل على 201
 */
...
class BookingApiTest {
  ...
  @Test
  @DisplayName(
      "POST /api/bookings then GET /api/bookings returns the created record, using JWT from /auth/login")
  void create_thenList() throws Exception {
    ...
    mvc.perform(
            get("/api/bookings").header("Authorization", "Bearer " + token).param("userId", "1"))
        .andExpect(status().isOk())
